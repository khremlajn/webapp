...
deploy_image() {
    ...
    docker tag webapp-repository:$CIRCLE_SHA1 792082350620.dkr.ecr.us-west-2.amazonaws.com/webapp-repository:$CIRCLE_SHA1
    docker push 792082350620.dkr.ecr.us-west-2.amazonaws.com/webapp-repository:$CIRCLE_SHA1
}
...
deploy_cluster() {
    ...
    make_task_def
    register_definition
    if [[ $(aws ecs update-service --cluster webapp-cluster --service webapp-ecs-service --task-definition $revision | \
